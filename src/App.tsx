...
import SalesToyota from './pages/SalesToyota/SalesToyota';
...
function AppContent() {
  ...
  const isToyotaRoute = location.pathname === '/sales_toyota';

  if (!user && !isToyotaRoute) {
    return <Login />;
  }

  return (
    <div className="app">
      {!isToyotaRoute && <Navbar />}
      <main>
        <Routes>
          ...
          <Route path="/sales_toyota" element={<SalesToyota />} />
        </Routes>
      </main>
    </div>
  );
